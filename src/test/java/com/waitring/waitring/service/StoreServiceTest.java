...
class StoreServiceTest {
     ...
     */
    Store generateStore() {
        List<StoreKeyword> storeKeywords = singletonList(StoreKeyword.builder().id(4L).keyword(generateKeyword()).build());
    ...
    void getStoreListByWord() {
        ...
        given(storeRepository.getStoreListByWord("버거")).willReturn(stores);
        ...
        assertThat(storeInfoList.get(0).getKeywords().size()).isEqualTo(1);

        // verify
        verify(storeRepository).getStoreListByWord(any(String.class));
    }
}